function out=BodyCaDynamics(mpfile)
%%%% BodyCaDynamics.m %%%%
...
%mpfile = input_file;%'parameters_body';%input('Specify name of model parameter file: ','s');
...
for k=1:np % loop over number of processes joining cell body
    ...
    Ccp{k} = Cc*ones(ng+2,3); % cytosolic Ca
    Cep{k} = Ce*ones(ng+2,1); % ER Ca
    InP3p{k} = InP3*ones(ng+2,3); % cytosolic InP3;
    CaCalBp{k} = CaCalB*ones(ng,3); % bound calcium value
    
    % quiescent InP3R states
    CI1p{k} = CI1*ones(ng,1);
    CI2p{k} = CI2*ones(ng,1);
    CI3p{k} = CI3*ones(ng,1);
    CI4p{k} = CI4*ones(ng,1);
    OI5p{k} = OI5*ones(ng,1);
    OI6p{k} = OI6*ones(ng,1);
    % gating variables
    mItp{k} = mIt*ones(ng,1);
    hItp{k} = hIt*ones(ng,1);
    
    % activated PKC
    PKCvp{k} = PKCv*ones(ng,1);
...
Ccb = Cc*ones(nc,nr); % cytosolic Ca
Ceb = Ce*ones(nc,nr); % ER Ca
InP3b = InP3*ones(nc,nr); % cytosolic InP3;
CaCalBb = CaCalB*ones(nc,nr); % bound calcium value
...
% activated PKC
PKCvb = PKCv*ones(nc,nr);

% % quiescent RyR  states: NOT USED IN CELL BODY
% if Drflag % if >= 1 RyR/compartment, initiate states @ each compartment
%     CR1b = CR1*ones(nc,nr);
%     CR2b = CR2*ones(nc,nr);
%     OR3b = OR3*ones(nc,nr);
...
% divisors for joint boundary value calculation
denCc = -1 / ( DCac*JPb + DCacb*JBb );
denCe = -1 / ( DCae*JPb + DCaeb*JBb );
denInP3 = -1 / ( DInP3*JPb + DInP3b*JBb );
...
for t = 0:dt:tend % loop on time
    ...
    for k=1:np % loop over processes attached to cell body
        ...
        % probability of open state for InP3Rs
        OpenIp{k} = OI5p{k} + OI6p{k};
        % Ca increment through open InP3R channels
        DCI = krI*OpenIp{k}.*(CeM-CcM(:,1));
    ...
    %   face terms
    for k=1:nr
        DCNb(:,k) = kpNbf(k).*Ccb(:,k)./(kpNC+Ccb(:,k));
    end
    %   sidewall terms
    ...
    %   face terms
    for k=1:nr
        DCPb(:,k) = kpPbf(k).*Cc2pw(:,k)./(kpPC2pw+Cc2pw(:,k));
    end
    %   sidewall terms
...
%     hItb = hI;
    ...
    %   face terms
    for k=1:nr
        DInP3Mib(:,k) = kmIfbf(k)*Ccb(:,k)./(1+kiPI*PKCvb(:,k));
    end
    %   sidewall terms
    ...
    Ccb = Ccb + DCcbv;
    for k=1:nr
        Ccb(:,k) = Ccb(:,k) + DClxbf(k); % PM leakage fluxes
    end
    ...
    for k=1:nc
        indx = mod( (k-kD):(k+kD-2), nc ) + 1;
        for kk=1:nr
            LC = LB{kk};
            DCcbC(k,kk) = DCacb * LC*Ccb(indx,kk); % cyto Ca
            DCebC(k,kk) = DCaeb * LC*Ceb(indx,kk); % ER Ca
            DInP3bC(k,kk) = DInP3b * LC*InP3b(indx,kk); % cyto InP3
        end
    end
    
    % radial diffusion
    %   non-junction elements: use RBW for sealed BC
    DCcbR(ksw,:) = DCacb * (RBW*Ccb(ksw,:)')'; % cyto Ca
    DCebR(ksw,:) = DCaeb * (RBW*Ceb(ksw,:)')'; % ER Ca
    DInP3bR(ksw,:) = DInP3b * (RBW*InP3b(ksw,:)')'; % cyto InP3
    %   junction elements: use RBJ for joint BC
    DCcbR(kjn,:) = DCacb * (RBJ*Ccb(kjn,:)')'; % cyto Ca
    DCebR(kjn,:) = DCaeb * (RBJ*Ceb(kjn,:)')'; % ER Ca
    DInP3bR(kjn,:) = DInP3b * (RBJ*InP3b(kjn,:)')'; % cyto InP3
    % contributions of boundary values for junction elements
    k0 = 0;
    for k=1:np % loop over junctions
        indx = k0 + (1:ncp);
        DCcbR(indx,end) = DCcbR(indx,end) + DCacb * RBJb*BVCc(k); % cyto Ca
        DCebR(indx,end) = DCebR(indx,end) + DCaeb * RBJb*BVCe(k); % ER Ca
        % cyto InP3
        DInP3bR(indx,end) = DInP3bR(indx,end) + DInP3b * RBJb*BVInP3(k);
        k0 = k0 + nct;
    end
    ...
    for k=1:np % loop over junctions
        ...
        BVCc(k) = BVCc(k) + DCacb.*sum(JB*Ccb(indx,:)')./ncp;
        BVCe(k) = BVCe(k) + DCaeb.*sum(JB*Ceb(indx,:)')./ncp;
        BVInP3(k) = BVInP3(k) + DInP3b.*sum(JB*InP3b(indx,:)')./ncp;
        k0 = k0 + nct;
...
save(outfile, 'Ccbt','Ccpt','Cebt','Cept','InP3bt','InP3pt',...
               'ng','np','nc','nct','ncp','nr','rb','rc','ld','data_time');
